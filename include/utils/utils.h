 ...
 * returns 1 + index of the last set bit of val, or 0 if val is 0
 */
static inline int fls32(uint32_t val) {
  return val == 0 ? 0 : fls32_unsafe(val);
}

static inline int fls64(uint64_t val) {
 ...
 * val is 0
 */
static inline uint32_t next_pow2_unsafe(uint32_t val) {
  return 1u << (fls32(val - 1));
}

static inline uint64_t next_pow2l_unsafe(uint64_t val) {
...
static inline uint64_t next_pow2l(uint64_t val) {
 ...
 * representation of val
 */
static inline uint32_t ceil_log10(uint32_t val) {
  uint32_t width = fls32(val);
  uint32_t guess = log10_pow2_guess[width];
  return guess + (val >= log10_ten_to[guess]);
}

static inline uint64_t ceil_log10l(uint64_t val) {
