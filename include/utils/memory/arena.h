...
class ArenaPtr {
 ...
 private:
  constexpr ArenaPtr(T* ptr, const Arena<T>& arena);
...
template <class T>
constexpr ArenaPtr<T>::ArenaPtr(T* ptr, const Arena<T>& arena)
    : ptr_val_(PtrToInt(ptr, arena.start())) {}

template <class T>
T* ArenaPtr<T>::get(const Arena<T>& arena) const {
