...
#define RB_DEFINE_FIND_LOC(name, cmp_fn)                                     \
  static rb_node_t *rb_find_##name##_loc(struct __int_rb_tree *tree,         \
                                         rb_node_t *node) {                  \
    ...
    while (root != LEAF) {                                                   \
      prev_node = root;                                                      \
      root =                                                                 \
          (cmp_fn(root, node) < 0 ? rb_get_right(root) : rb_get_left(root)); \
    }                                                                        \
    return prev_node;                                                        \
  }
...
#define RB_DEFINE_UPPER_BOUND(name, cmp_fn)                           \
  static rb_node_t *rb_upper_bound_##name(struct __int_rb_tree *tree, \
                                          rb_node_t *node) {          \
...
#define RB_DEFINE_LOWER_BOUND(name, cmp_fn)                           \
  static rb_node_t *rb_lower_bound_##name(struct __int_rb_tree *tree, \
                                          rb_node_t *node) {          \
...
#define RB_DEFINE_FIND_HELPER(name, cmp_fn)                     \
  static rb_node_t *_rb_find_##name##_helper(rb_node_t *root,   \
                                             rb_node_t *node) { \
    while (root != LEAF) {                                      \
...
#define RB_DEFINE_FIND(name)                                   \
  static rb_node_t *rb_find_##name(struct __int_rb_tree *tree, \
                                   rb_node_t *node) {          \
    rb_node_t *root;                                           \
...
#define rb_for_each_mod(tree, node)                             \
  do {                                                          \
    ...
    for (rb_node_t *__rb_succ_##node =                          \
             ((node) == LEAF ? LEAF : rb_find_succ(node));      \
         (node) != LEAF; (node) = __rb_succ_##node,             \
                   __rb_succ_##node = (__rb_succ_##node == LEAF \
                                           ? LEAF               \
...
#define rb_for_each_mod_rev(tree, node)                         \
  do {                                                          \
    ...
    for (rb_node_t *__rb_pred_##node =                          \
             ((node) == LEAF ? LEAF : rb_find_pred(node));      \
         (node) != LEAF; (node) = __rb_pred_##node,             \
                   __rb_pred_##node = (__rb_pred_##node == LEAF \
                                           ? LEAF               \
...
#define RB_DEFINE_SCALAR_FIND_HELPER(name, s_type)                    \
  static rb_node_t *_rb_contains_##name##_helper(rb_node_t *root,     \
                                                 s_type val) {        \
    while (root != LEAF) {                                            \
      s_type root_val = rb_##name##_val(root);                        \
      if (root_val == val)                                            \
