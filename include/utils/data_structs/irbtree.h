...
#define IRB_DEFINE_FIND_LOC(name, less_fn)                                     \
  static irb_node *irb_find_##name##_loc(struct __int_irb_tree *tree,          \
                                         irb_node *node) {                     \
...
#define IRB_DEFINE_FIND_INC(name, less_fn)                              \
  static irb_node *__irb_find_##name##_inc(struct __int_irb_tree *tree, \
                                           irb_node *node) {            \
    irb_node *prev_node = ILEAF;                                        \
    irb_node *root = irb_get_root(tree);                                \
    while (root != ILEAF) {                                             \
...
#define IRB_DEFINE_UPPER_BOUND(name, less_fn)                          \
  static irb_node *irb_upper_bound_##name(struct __int_irb_tree *tree, \
                                          irb_node *node) {            \
...
#define IRB_DEFINE_LOWER_BOUND(name, less_fn)                          \
  static irb_node *irb_lower_bound_##name(struct __int_irb_tree *tree, \
                                          irb_node *node) {            \
...
#define IRB_DEFINE_CONTAINS(name, less_fn)                    \
  static int irb_contains_##name(struct __int_irb_tree *tree, \
                                 irb_node *node) {            \
    irb_node *root;                                           \
...
int _irb_insert_helper(struct __int_irb_tree *tree, irb_node *node,
                       irb_node *p);
...
#define IRB_DEFINE_GET_IDX(name, less_fn)                          \
  static irb_idx_t irb_get_idx_##name(struct __int_irb_tree *tree, \
                                      irb_node *n) {               \
    irb_node *root = irb_get_root(tree);                           \
    irb_idx_t idx = 0;                                             \
    while (root != ILEAF) {                                        \
...
#define IRB_DEFINE_GET_INS_IDX(name, less_fn)                          \
  static irb_idx_t irb_get_ins_idx_##name(struct __int_irb_tree *tree, \
                                          irb_node *n) {               \
    irb_node *root = irb_get_root(tree);                               \
    irb_idx_t idx = 0;                                                 \
    while (root != ILEAF) {                                            \
