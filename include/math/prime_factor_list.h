...
int
prime_factor_list_init(prime_factor_list_t*, uint64_t max);

void
prime_factor_list_free(prime_factor_list_t*);
...
public:
	...
	public:
		factor_iterator(uint64_t n, const std::vector<uint64_t>* factors) : i(n), factors(factors) {
			_calc_next_factor();
		}

		// used only for factor_end()
		constexpr factor_iterator(const uint64_t n) : i(n), factor_repeats({ 1, 0 }), factors(nullptr) {}
