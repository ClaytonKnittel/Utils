...
int rtree_init(rtree_t* tree, uint32_t m_min, uint32_t m_max) {
  if (m_min < 2) {
...
static void _rtree_leaf_free(rtree_leaf_t* leaf) {
  free(leaf);
}
...
bool rtree_rect_intersects(const rtree_rect_t* a, const rtree_rect_t* b) {
  return !((a->ux < b->lx) || (b->ux < a->lx) || (a->uy < b->ly) ||
           (b->uy < a->ly));
}
...
bool rtree_rect_contains(const rtree_rect_t* p, const rtree_rect_t* r) {
  return ((p->lx <= r->lx) && (r->ux <= p->ux) && (p->ly <= r->ly) &&
          (r->uy <= p->uy));
}
...
rtree_coord_t rtree_rect_area_increase(const rtree_rect_t* base,
  ...
  area_inc = ((b.ux - b.lx) * (b.uy - b.ly)) -
             ((base->ux - base->lx) * (base->uy - base->ly));
 ...
 */
static uint32_t _min_overlap_cost_child(rtree_node_base_t** children,
                                        uint32_t n, rtree_rect_t* rect) {
  struct cost {
  ...
  for (uint32_t i = 1; i < n; i++) {
    ...
    for (uint32_t _idx = 0; _idx < n - 1; _idx++) {
      ...
      cur.overlap_inc -=
          rtree_rect_overlap(&children[i]->bb, &children[idx]->bb);
    }
 ...
 */
static uint32_t _min_area_increase_child(rtree_node_base_t** children,
                                         uint32_t n, rtree_rect_t* rect) {
  struct cost {
    rtree_coord_t area_inc;
    rtree_coord_t area;
 ...
 * box of each
 */
static void _shrunk_node_bb(rtree_node_base_t* node) {
  rtree_rect_t new_bb;
 ...
 * being removed)
 */
static void _shrink_leaf_bb(rtree_leaf_t* leaf) {
  if (leaf->base.n == 0) {
...
DEFINE_CSORT_DEFAULT_FNS_NAMED_16(rtree_rect_t*, rtree_rect_x, rect_x_sort_cmp,
                                  rect_x_sort_cswap)
DEFINE_CSORT_DEFAULT_FNS_NAMED_16(rtree_rect_t*, rtree_rect_y, rect_y_sort_cmp,
                                  rect_y_sort_cswap)
 ...
 */
static bool _determine_split(rtree_t* tree, uint32_t n, rtree_rect_t** x_sort,
  ...
  rev_rects = (rtree_rect_t*) alloca(n_dists * sizeof(rtree_rect_t));
  {
    ...
    for (; i >= m_min; i--) {
      rtree_rect_combine(&rev_rects[i - m_min], &rev_rects[i - m_min + 1],
                         x_sort[i]);
    }
  }
  ...
  // the pre-calculated higher ones
  {
    ...
    ")>\n" "  <(%" PRId64 ", %" PRId64 "), (%" PRId64 ", %" PRId64 ")>
    (%llu)\n", i, x_bb.lx, x_bb.ly, x_bb.ux, x_bb.uy, rev_rects[0].lx,
    rev_rects[0].ly, rev_rects[0].ux, rev_rects[0].uy, rtree_rect_margin(&x_bb)
    + rtree_rect_margin(&rev_rects[0]));
                    */
    ...
    for (; i < n - m_min; i++) {
      ...
      rtree_coord_t overlap =
          rtree_rect_overlap(&x_bb, &rev_rects[i - m_min + 1]);
      rtree_coord_t area =
          rtree_rect_area(&x_bb) + rtree_rect_area(&rev_rects[i - m_min + 1]);

      /*
      printf("x at %u,\n"
                      "  <(%" PRId64 ", %" PRId64 "), (%" PRId64 ", %" PRId64
      ")>\n" "  <(%" PRId64 ", %" PRId64 "), (%" PRId64 ", %" PRId64 ")>
      (%llu)", i, x_bb.lx, x_bb.ly, x_bb.ux, x_bb.uy, rev_rects[i - m_min +
      1].lx, rev_rects[i - m_min + 1].ly, rev_rects[i - m_min + 1].ux,
      rev_rects[i - m_min + 1].uy, rtree_rect_margin(&x_bb) +
      rtree_rect_margin(&rev_rects[i - m_min + 1]));
                      */

      if (overlap < opt_x.overlap ||
          (overlap == opt_x.overlap && area < opt_x.area)) {
  ...
  // the pre-calculated higher ones
  {
    ...
    printf("y at %u,\n"
                    "  <(%" PRId64 ", %" PRId64 "), (%" PRId64 ", %" PRId64
    ")>\n" "  <(%" PRId64 ", %" PRId64 "), (%" PRId64 ", %" PRId64 ")>
    (%llu)\n", i, y_bb.lx, y_bb.ly, y_bb.ux, y_bb.uy, rev_rects[0].lx,
    rev_rects[0].ly, rev_rects[0].ux, rev_rects[0].uy, rtree_rect_margin(&y_bb)
    + rtree_rect_margin(&rev_rects[0]));
                    */
    ...
    for (; i < n - m_min; i++) {
      ...
      rtree_coord_t overlap =
          rtree_rect_overlap(&y_bb, &rev_rects[i - m_min + 1]);
      rtree_coord_t area =
          rtree_rect_area(&y_bb) + rtree_rect_area(&rev_rects[i - m_min + 1]);

      /*
      printf("y at %u,\n"
                      "  <(%" PRId64 ", %" PRId64 "), (%" PRId64 ", %" PRId64
      ")>\n" "  <(%" PRId64 ", %" PRId64 "), (%" PRId64 ", %" PRId64 ")>
      (%llu)", i, y_bb.lx, y_bb.ly, y_bb.ux, y_bb.uy, rev_rects[i - m_min +
      1].lx, rev_rects[i - m_min + 1].ly, rev_rects[i - m_min + 1].ux,
      rev_rects[i - m_min + 1].uy, rtree_rect_margin(&y_bb) +
      rtree_rect_margin(&rev_rects[i - m_min + 1]));
                      */

      if (overlap < opt_y.overlap ||
          (overlap == opt_y.overlap && area < opt_y.area)) {
  ...
  *y_cost = opt_y;
  ...
  return x_margin < y_margin;  // TODO bit parity || (x_sum == y_sum &&
                               // (__builtin_(x_sum ^ y_sum) & 1)))
}

static rtree_node_base_t* _split_leaf(rtree_t* tree, rtree_leaf_t* node,
                                      rtree_el_t* to_add) {
...
struct node_dist {
  /*
   * 4 * squared distance of the center of node's bb to the center of its
   * parent's bb
   *
...
#define node_dist_sort_cmp(a, b) ((a).dist2 < (b).dist2)
...
DEFINE_CSORT_DEFAULT_FNS_NAMED_16(struct node_dist, node_dist,
                                  node_dist_sort_cmp, node_dist_sort_cswap)
...
static int _do_reinsert_leaf(rtree_t* tree, rtree_leaf_t* parent,
                             int_set_t reinserted_levels, rtree_el_t* el_ptr) {
...
static void _do_reinsert(rtree_t* tree, rtree_node_t* parent,
                         int_set_t reinserted_levels, uint64_t depth,
                         rtree_node_base_t* to_add) {
  uint32_t m_max = tree->m_max;
  uint32_t reinsert_p = tree->reinsert_p;
 ...
 */
static int __continue_split(rtree_t* tree, rtree_node_base_t* n,
                            int_set_t reinserted_levels, uint64_t depth,
                            const rtree_rect_t* rect) {
...
/*
 * reinserts an inner node at the specified depth, width depth = 0 being the
 * root
 *
 ...
 */
static int _do_insert_under_node(rtree_t* tree, rtree_el_t* el_ptr,
                                 int_set_t reinserted_levels,
...
int rtree_insert(rtree_t* tree, rtree_el_t* el_ptr) {
  int_set_t reinserted_levels;
 ...
 */
static void _reinsert_node_children(rtree_t* tree, rtree_node_t* node,
                                    int_set_t reinserted_levels, uint64_t depth,
                                    uint32_t delete_idx) {
  uint64_t inv_depth = tree->depth - depth;
  ...
  for (uint32_t i = 0; i < delete_idx; i++) {
    _do_inner_insert(tree, node->children[i], reinserted_levels,
                     tree->depth - inv_depth);
  }
 ...
 */
static int _reinsert_leaf_children(rtree_t* tree, rtree_leaf_t* leaf,
                                   int_set_t reinserted_levels,
                                   uint32_t delete_idx) {
  // remove the node before reinserting it into the tree
 ...
 */
static int _do_delete_element(rtree_t* tree, rtree_el_t* el_ptr,
                              int_set_t reinserted_levels) {
...
int rtree_delete(rtree_t* tree, rtree_el_t* el) {
  int_set_t reinserted_levels;
  int_set_inita(reinserted_levels, tree->depth);
...
bool rtree_foreach(const rtree_t* tree, rtree_foreach_cb callback,
                   void* udata) {
...
static rtree_el_t* _rtree_find_exact_leaf(const rtree_leaf_t* leaf,
                                          const rtree_rect_t* rect) {
...
static rtree_el_t* _rtree_find_exact(const rtree_node_t* node,
                                     const rtree_rect_t* rect, int depth) {
...
rtree_el_t* rtree_find_exact(const rtree_t* tree, const rtree_rect_t* rect) {
...
static bool _rtree_intersects_leaf(const rtree_t* tree,
                                   const rtree_leaf_t* leaf,
                                   const rtree_rect_t* rect,
...
static bool _rtree_intersects(const rtree_t* tree, const rtree_node_t* node,
                              const rtree_rect_t* rect,
                              rtree_intersects_cb callback, void* udata,
                              int depth) {
  for (uint32_t i = 0; i < node->base.n; i++) {
...
bool rtree_intersects_foreach(const rtree_t* tree, const rtree_rect_t* rect,
                              rtree_intersects_cb callback, void* udata) {
  rtree_node_base_t* n = tree->root;
  uint64_t depth = tree->depth;
...
static int _do_move(rtree_t* tree, rtree_el_t* el_ptr,
                    const rtree_rect_t* new_rect) {
...
int rtree_move(rtree_t* tree, rtree_el_t* el, const rtree_rect_t* new_rect) {
...
static void rtree_print_leaf(const rtree_leaf_t* leaf, int depth) {
  printf("%*s{ ((%" PRId64 ", %" PRId64 "), (%" PRId64 ", %" PRId64 "))",
         2 * depth, "", leaf->base.bb.lx, leaf->base.bb.ly, leaf->base.bb.ux,
         leaf->base.bb.uy);
  for (uint32_t i = 0; i < leaf->base.n; i++) {
    printf("\n%*s<(%" PRId64 ", %" PRId64 "), (%" PRId64 ", %" PRId64 ")>",
           2 * depth + 2, "", leaf->elements[i]->bb.lx,
           leaf->elements[i]->bb.ly, leaf->elements[i]->bb.ux,
           leaf->elements[i]->bb.uy);
  }
  printf("\n%*s}", 2 * depth, "");
}
...
void rtree_print(const rtree_t* tree) {
...
static void _rtree_validate_leaf(const rtree_t* tree, const rtree_leaf_t* leaf,
...
static void _rtree_validate_node(const rtree_t* tree,
                                 const rtree_node_base_t* n,
                                 const rtree_node_base_t* parent,
                                 uint64_t depth) {
  if (depth == 0) {
...
void rtree_validate(const rtree_t* tree) {
