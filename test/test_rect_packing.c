...
static void _rect_packing_validate(const rect_packing_t* packing,
                                   vector_t* els) {
  ...
  for (uint64_t i = 0; i < vector_size(els); i++) {
    ...
    if (verbose) {
      printf("el: (%llu, %llu), (%llu x %llu) (%u) (%s)\n", el->lx, el->ly,
             el->w, el->h, el->bin_idx, boolstr(el->rotated));
    }
...
static void _reshuffle_els(rect_packing_t* packing, vector_t* els) {
